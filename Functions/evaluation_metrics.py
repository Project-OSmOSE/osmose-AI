...
def plot_4_curves(Recall, Precision, FP_rate, TP_rate, FN_rate, ProbabilityCost, NormalizedExpectedCost, savepath = None, color='b', figsize=(8,8), title=' '):
    ...
    ax4.plot(ProbabilityCost, NormalizedExpectedCost, lw=0.1, color=color)
    ax4.plot([0,1],[0,1], 'k')
    ax4.plot([0,1],[1,0], 'k')
    ax4.plot(ProbabilityCost, np.min(NormalizedExpectedCost, axis=1), lw=4, color=color)
    ax4.grid()
    ax4.set_xlabel('Probability Cost')
    ax4.set_ylabel('Normalized Expected Cost')  
    ax4.set_ylim([0,0.5]) 
    ax4.set_xlim([0,1]) 
